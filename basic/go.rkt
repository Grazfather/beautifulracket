#lang br
(require "struct.rkt" "line.rkt" "misc.rkt")
(provide b-end b-goto b-gosub b-return b-for b-next)

(define (b-end) (raise (end-program-signal)))

(define (b-goto num-expr)
  (raise (change-line-signal num-expr)))

(define return-ccs empty)

; To evaluate a gosub we create a continuation and push it onto the return-ccs
; stack and then goto the target of the gosub
(define (b-gosub num-expr)
  (let/cc here-cc
    (push! return-ccs here-cc)
    (b-goto num-expr)))

; On return we pop the continuation of the stack and continue, unless the stack
; is empty, in which case we raise an error
(define (b-return)
  (when (empty? return-ccs)
    (raise-line-error "return without gosub"))
  (define top-cc (pop! return-ccs))
  (top-cc (void)))

; We use a hash map to track loop start points. The values are functions that
; update the next value, see if the loop should continue, and if so, continue.
; If the loop is done, then it removes itself from the hash map
(define next-funcs (make-hasheq))

(define-macro-cases b-for
   [(_ LOOP-ID START END) #'(b-for LOOP-ID START END 1)]
   [(_ LOOP-ID START END STEP)
    #'(b-let LOOP-ID
             (let/cc loop-cc
               (hash-set! next-funcs
                          'LOOP-ID
                          (lambda ()
                            (define next-val
                              (+ LOOP-ID STEP))
                            (if (next-val . in-closed-interval? . START END)
                              (loop-cc next-val)
                              (hash-remove! next-funcs 'LOOP-ID))))
               START))])

(define (in-closed-interval? x start end)
  ((if (< start end) <= >=) start x end))


; b-next simply looks in the next-funcs hash map for a function and calls it,
; continuing at the top if it should. An error is raised if the label for the
; next is not found
(define-macro (b-next LOOP-ID)
  #'(begin
      (unless (hash-has-key? next-funcs 'LOOP-ID)
        (raise-line-error
         (format "`next ~a` without for" 'LOOP-ID)))
      (define func (hash-ref next-funcs 'LOOP-ID))
      (func)))
